...
export const useUploadResume = () => {
  ...
  return useMutation({
    mutationFn: ({ id, file, onUploadProgress }: UploadResumePayload) =>
      uploadResumeFile({ id, file, onUploadProgress }),
    onSuccess: () => {
...
export const useUploadCoverLetterTemplate = () => {
  ...
  return useMutation({
    mutationFn: ({ file, onUploadProgress }: UploadCoverLetterTemplatePayload) =>
      uploadCoverLetterTemplate({ file, onUploadProgress }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: fileKeys.coverLetterTemplate() });
    },
    meta: {
      getErrorMessage,
    },
...
export const useUploadOtherFile = () => {
  ...
  return useMutation({
    mutationFn: ({ id, file, onUploadProgress }: UploadOtherFilePayload) =>
      uploadOtherFiles({ id, file, onUploadProgress }),
    onSuccess: () => {
...
export const useCoverLetterFile = () => {
  return useMutation({
    mutationFn: ({ id, email, company, content }: CoverLetterFilePayload) =>
      fetchCoverLetterFile({ id, email, company, content }),
    meta: {
      getErrorMessage,
    },
  });
...
export const useMergeFile = () => {
  return useMutation({
    mutationFn: ({ coverletter, email, company, content }: MergeFilePayload) =>
      fetchMergeFile({ coverletter, email, company, content }),
    meta: {
      getErrorMessage,
    },
  });
